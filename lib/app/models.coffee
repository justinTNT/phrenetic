	...
	for schema in schemas.all()
		...
		for pathName, path of schema.definition
			...
			if not path.type
				...
				if _.isArray(path)
					if (ref = path[0].ref)
						properties[pathName] = DS.hasMany "App.#{ref}", defaultValue: []
					else properties[pathName] = DS.attr 'array', defaultValue: []
			else
				properties[pathName] =
					switch path.type
						...
						else throw new Error "unrecognised type #{path.type}"
